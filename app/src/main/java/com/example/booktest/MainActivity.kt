...
class MainActivity : AppCompatActivity(), Comunicator {
    ...
    override fun sendDataToSearch(bookTitle: String, bookFilter: String, bookType: String) {
        supportFragmentManager.beginTransaction().replace(R.id.fragment_display, DisplayFragment.newInstance(bookTitle, bookFilter, bookType)).addToBackStack(null).commit()
    }
}